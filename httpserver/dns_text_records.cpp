...
static std::string get_dns_record(const char* url, std::error_code& ec) {
    ...
    if (response == -1) {
        WORKTIPS_LOG(warn, "res_query failed while retrieving dns entry");
    ...
    if (ns_initparse(query_buffer, response, &nsMsg) == -1) {
        WORKTIPS_LOG(warn, "ns_initparse failed while retrieving dns entry");
        ec = std::make_error_code(std::errc::bad_message);
        return data;
    }
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (ns_parserr(&nsMsg, ns_s_an, i, &rr) == -1) {
            WORKTIPS_LOG(warn, "ns_parserr failed while parsing dns entry");
            ec = std::make_error_code(std::errc::bad_message);
            return data;
        }
...
std::vector<pow_difficulty_t> query_pow_difficulty(std::error_code& ec) {
    WORKTIPS_LOG(debug, "Querying PoW difficulty...");
    ...
    try {
        ...
        WORKTIPS_LOG(warn, "JSON parsing of PoW data failed: {}", e.what());
        ec = std::make_error_code(std::errc::bad_message);
        return new_history;
    }
}
...
static std::string query_latest_version() {
    WORKTIPS_LOG(debug, "Querying Latest Version...");
...
static bool parse_version(const std::string& str, version_t& version_out) {
    ...
    if (strs.size() != 3) {
        WORKTIPS_LOG(warn, "Invalid format for the Storage Server version!");
        return false;
    }
...
void check_latest_version() {
    ...
    if (latest_version_str.empty()) {
        WORKTIPS_LOG(warn, "Failed to retrieve or parse the latest version number "
                       "from DNS record");
        return;
    }
    ...
    if (!parse_version(latest_version_str, latest_version)) {
        WORKTIPS_LOG(warn, "Could not parse the latest version: {}",
                 latest_version_str);
        return;
    }
