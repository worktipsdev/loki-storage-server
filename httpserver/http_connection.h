...
constexpr auto WORKTIPS_SENDER_SNODE_PUBKEY_HEADER = "X-Worktips-Snode-PubKey";
constexpr auto WORKTIPS_SNODE_SIGNATURE_HEADER = "X-Worktips-Snode-Signature";
constexpr auto WORKTIPS_SENDER_KEY_HEADER = "X-Sender-Public-Key";
constexpr auto WORKTIPS_TARGET_SNODE_KEY = "X-Target-Snode-Key";
...
class WorktipsdClient {
    ...
    std::string worktipsd_rpc_ip_;
    const uint16_t worktipsd_rpc_port_;

  public:
    WorktipsdClient(boost::asio::io_context& ioc, std::string ip, uint16_t port);
    void make_worktipsd_request(boost::string_view method,
                            const nlohmann::json& params,
                            http_callback_t&& cb) const;
    void make_custom_worktipsd_request(const std::string& daemon_ip,
                                   const uint16_t daemon_port,
                                   boost::string_view method,
                                   const nlohmann::json& params,
                                   http_callback_t&& cb) const;
    // Synchronously fetches the private key from worktipsd.  Designed to be called *before* the
...
constexpr const char *error_string(SNodeError err) {
    switch (err) {
        case worktips::SNodeError::NO_ERROR:
            return "NO_ERROR";
        case worktips::SNodeError::ERROR_OTHER:
            return "ERROR_OTHER";
        case worktips::SNodeError::NO_REACH:
            return "NO_REACH";
        case worktips::SNodeError::HTTP_ERROR:
            return "HTTP_ERROR";
        default:
            return "[UNKNOWN]";
    }
...
template <>
struct formatter<worktips::SNodeError> {
    ...
    auto format(const worktips::SNodeError& err, FormatContext& ctx) {
