...
void make_https_request(boost::asio::io_context& ioc,
    ...
    if (sn_address == "0.0.0.0") {
        WORKTIPS_LOG(debug, "Could not initiate request to snode (we don't know "
                       "their IP yet).");
...
void make_https_request(boost::asio::io_context& ioc, const std::string& url,
    ...
    auto resolve_handler = [&ioc, req, query, cb = std::move(cb)] (
        ...
        if (ec) {
            WORKTIPS_LOG(error, "DNS resolution error for {}: {}", query, ec.message());
            return;
        }
...
static std::string x509_to_string(X509* x509) {
    ...
    if (!bio_out) {
        WORKTIPS_LOG(critical, "Could not allocate openssl BIO");
        return "";
    }
    if (!PEM_write_bio_X509(bio_out, x509)) {
        WORKTIPS_LOG(critical, "Could not write x509 cert to openssl BIO");
        return "";
    }
    BUF_MEM* bio_buf;
    ...
    if (!BIO_free(bio_out)) {
        WORKTIPS_LOG(critical, "Could not free openssl BIO");
    }
    return pem;
}
...
void HttpsClientSession::start() {
    ...
    if (!SSL_set_tlsext_host_name(stream_.native_handle(), "service node")) {
        ...
        WORKTIPS_LOG(critical, "{}", ec.message());
        return;
    }
...
void HttpsClientSession::on_connect() {
    WORKTIPS_LOG(trace, "on connect, connection idx: {}", this->connection_idx);
...
void HttpsClientSession::on_handshake(boost::system::error_code ec) {
    if (ec) {
        WORKTIPS_LOG(error, "Failed to perform a handshake with {}: {}",
...
void HttpsClientSession::on_write(error_code ec, size_t bytes_transferred) {
    ...
    if (ec) {
        WORKTIPS_LOG(error, "Https error on write, ec: {}. Message: {}", ec.value(),
                 ec.message());
...
bool HttpsClientSession::verify_signature() {
    ...
    const auto it = res_.find(WORKTIPS_SNODE_SIGNATURE_HEADER);
    if (it == res_.end()) {
        WORKTIPS_LOG(warn, "no signature found in header from {}",
...
void HttpsClientSession::on_read(error_code ec, size_t bytes_transferred) {
    ...
    if (!ec || (ec == http::error::end_of_stream)) {
        ...
        if (http::to_status_class(res_.result_int()) ==
            ...
            if (server_pub_key_b32z_ && !verify_signature()) {
                WORKTIPS_LOG(debug, "Bad signature from {}", *server_pub_key_b32z_);
    ...
    if (ec && ec != boost::system::errc::not_connected) {
        ...
        WORKTIPS_LOG(error, "ec: {}. Message: {}", ec.value(), ec.message());
        return;
    }
...
void HttpsClientSession::on_shutdown(boost::system::error_code ec) {
    if (ec == boost::asio::error::eof) {
        ...
        WORKTIPS_LOG(debug, "could not shutdown stream gracefully: {} ({})",
                 ec.message(), ec.value());
    }
