...
void serialize_message(std::string& res, const T& msg) {
    ...
    WORKTIPS_LOG(trace, "serialized message: {}", msg.data);
}
...
std::vector<message_t> deserialize_messages(const std::string& blob) {
    ...
    WORKTIPS_LOG(trace, "=== Deserializing ===");
    ...
    while (!slice.empty()) {
        ...
        /// Deserialize PK
        auto pk = deserialize_string(slice, worktips::get_user_pubkey_size());
        if (!pk) {
            WORKTIPS_LOG(debug, "Could not deserialize pk");
            return {};
        }
        ...
        if (!hash) {
            WORKTIPS_LOG(debug, "Could not deserialize hash");
            return {};
        }
        ...
        if (!data) {
            WORKTIPS_LOG(debug, "Could not deserialize data");
            return {};
        }
        ...
        if (!ttl) {
            WORKTIPS_LOG(debug, "Could not deserialize ttl");
            return {};
        }
        ...
        if (!timestamp) {
            WORKTIPS_LOG(debug, "Could not deserialize timestamp");
            return {};
        }
        ...
        if (!nonce) {
            WORKTIPS_LOG(debug, "Could not deserialize nonce");
            return {};
        }

        WORKTIPS_LOG(trace, "Deserialized data: {}", *data);

        WORKTIPS_LOG(trace, "pk: {}, msg: {}", *pk, *data);

        result.push_back({*pk, *data, *hash, *ttl, *timestamp, *nonce});
    }

    WORKTIPS_LOG(trace, "=== END ===");
